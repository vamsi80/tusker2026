...
export default function Wipro() {
  ...
  const go = (d: number) => setN((s) => Math.min(TOTAL, Math.max(1, s + d)));
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black flex items-center justify-center select-none">
      <div className="relative aspect-video w-[min(100vw,calc(100vh*16/9))]">
        ...
        {/* preload next slide */}
        {n < TOTAL && <link rel="preload" as="image" href={`/wipro/${n + 1}.jpg`} />}
      </div>

      <button onClick={() => go(-1)} disabled={n === 1} aria-label="Previous slide" className={`${btn} left-4`}><ChevronLeft /></button>
      <button onClick={() => go(1)} disabled={n === TOTAL} aria-label="Next slide" className={`${btn} right-4`}><ChevronRight /></button>
